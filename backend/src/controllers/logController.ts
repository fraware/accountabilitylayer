...
const createLogBodySchema = z.object({
  ...
  status: z.enum(['success', 'failure', 'anomaly']).optional(),
});
...
export async function createLog(req: Request, res: Response): Promise<void> {
  try {
    ...
    const logData = { ...parsed.data } as Record<string, unknown>;
...
export async function createBulkLogs(req: Request, res: Response): Promise<void> {
  try {
    ...
    const validatedLogs: Record<string, unknown>[] = [];
    for (const item of logs) {
      ...
      const entry = { ...p.data } as Record<string, unknown>;
...
export async function getLogsByAgent(req: Request, res: Response): Promise<void> {
  try {
...
export async function getLogStep(req: Request, res: Response): Promise<void> {
  try {
...
export async function updateLogReview(req: Request, res: Response): Promise<void> {
  try {
    const { reviewed, review_comments } = req.body as {
      reviewed?: boolean;
    ...
    const eventResult = await eventBus.publish(
      'logs.update',
      {
        logId: log._id,
        updates: {
...
export async function searchLogs(req: Request, res: Response): Promise<void> {
  try {
    ...
    const keywordTrimmed =
      keyword !== undefined && String(keyword).trim() !== ''
        ? String(keyword).trim()
        : '';
    if (keywordTrimmed) {
    ...
    if (from_date || to_date) {
      ...
      if (from_date) (query.timestamp as Record<string, Date>).$gte = new Date(String(from_date));
      if (to_date) (query.timestamp as Record<string, Date>).$lte = new Date(String(to_date));
    } else {
      query.timestamp = {
...
export async function summaryLogs(req: Request, res: Response): Promise<void> {
  try {
    ...
    const agentSummary = agent_id ? summary.find((s: { _id: string }) => s._id === agent_id) : summary;

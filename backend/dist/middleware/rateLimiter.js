...
class RateLimiterService {
    ...
    async initialize(options = {}) {
        try {
            const { redisUrl = process.env.REDIS_URL || 'redis://localhost:6379', enableRedis = true } = options;
            if (enableRedis) {
    ...
    createLimiters() {
        ...
        this.limiters.set('bulk', rateLimit({
            ...
            max: (req) => {
                const user = req.user;
                if (!user)
                    return 1; // Anonymous users limited to 1 bulk operation per hour
                switch (user.role) {
                    case 'admin':
                        return 100;
                    case 'moderator':
        ...
        this.limiters.set('search', rateLimit({
            ...
            max: (req) => {
                const user = req.user;
                if (!user)
                    return 10; // Anonymous users limited to 10 searches per 5 minutes
                switch (user.role) {
                    case 'admin':
                        return 1000;
                    case 'moderator':
        ...
        this.limiters.set('websocket', rateLimit({
            ...
            max: (req) => {
                const user = req.user;
                if (!user)
                    return 3; // Anonymous users limited to 3 connections per minute
                switch (user.role) {
                    case 'admin':
                        return 100;
                    case 'moderator':
